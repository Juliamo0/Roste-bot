...
- 🎙️ **ระบบเสียงรอสเต้ (pipeline พร้อม ยังไม่เข้าบอท)** — ข้อความ → edge-tts → ffmpeg adjust → RVC (RVC model) → เสียงรอสเต้ — ทดสอบ standalone แล้ว รอ integrate เข้าบอทในเฟส 3
...
```
ข้อความ → edge-tts (th-TH-PremwadeeNeural) → ffmpeg (+5.292 semitones, speed 0.9×) → RVC (RVC model) → .wav
```
