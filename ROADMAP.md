...
- [x] pipeline: ข้อความ → edge-tts (PremwadeeNeural) → ffmpeg (pitch +5.292, speed 0.9×) → RVC (RVC model) → .wav
...
ยังไม่ตัดสินใจ ต้องทดสอบก่อน — flow เหมือนเดิม: TTS → RVC (RVC) → เสียงรอสเต้
...
**วิธีทดสอบ Gemini ก่อนตัดสินใจ:** สร้างเสียงไทยใน AI Studio → ผ่าน RVC RVC → ฟังว่าเป็นธรรมชาติขึ้นไหม
