...
async def _tool_get_power_outage(args: dict, mem: dict) -> str:
    ...
    return (f"[ข้อมูลประกาศตัดไฟด้านล่างเป็นของ 'จังหวัด{HOME_PROVINCE_NAME}' เท่านั้น "
            f"เครื่องมือนี้เช็คได้เฉพาะจังหวัดนี้ ไม่มีข้อมูลจังหวัดอื่นเลย "
            f"** สำคัญ: ถ้าผู้ใช้ถามถึงจังหวัดอื่นที่ไม่ใช่{HOME_PROVINCE_NAME} ให้บอกตรงๆ ว่ารอสเต้เช็ค"
            f"ได้แค่{HOME_PROVINCE_NAME} ไม่มีข้อมูลจังหวัดที่เขาถาม ห้ามเอาข้อมูล{HOME_PROVINCE_NAME}"
            f"ด้านล่างไปตอบแทนเด็ดขาด ** "
            f"ถ้าเขาถาม{HOME_PROVINCE_NAME}หรือ 'แถวบ้าน' ให้เล่าข้อมูลด้านล่างด้วยน้ำเสียงเป็นกันเองและห่วงใย "
            "ไม่ใช่อ่านลิสต์ดิบ บอกวัน เวลา บริเวณ เรียงจากใกล้สุด ถ้ามีหลายรายการสรุปกระชับ "
            "เตือนเตรียมตัว (ชาร์จแบต/สำรองน้ำ) สั้นๆ ถ้าไม่มีประกาศก็บอกตามนั้น ห้ามแต่งวันเวลา/สถานที่เพิ่มเอง "
            f"ปิดท้ายบอกว่าข้อมูลจากการไฟฟ้าส่วนภูมิภาค]\n\n[ข้อมูลภายใน — จังหวัด{HOME_PROVINCE_NAME}]\n" + info)
