...
# จำนวนบทสรุปสูงสุดที่เก็บไว้ต่อคน (history ล้นแล้วสรุป 1 บรรทัดแทนทิ้ง)
MAX_SUMMARIES = 20
...
def build_extract_prompt(user_message: str) -> str:
    ...
    return (
        ...
        "- เขียนแต่ละข้อสั้นๆ กระชับ เป็นภาษาไทย (เช่น \"ชื่อจูเลีย\", \"อยู่ชุมพร\", \"ทำโปรเจกต์หุ่นยนต์\")\n"
...
def build_summary_prompt(pairs: list) -> str:
    """สร้าง prompt ให้โมเดลสรุปบทสนทนาเก่าเป็น 1 ประโยคสั้นๆ"""
    convo = "\n".join(
        f"{'ผู้ใช้' if m['role'] == 'user' else 'รอสเต้'}: {m.get('content', '')}"
        for m in pairs
    )
    return (
        "สรุปบทสนทนาต่อไปนี้เป็น 1 ประโยคสั้นๆ ภาษาไทย ว่าคุยเรื่องอะไร "
        "ตอบมาแค่ประโยคสรุปเดียวเท่านั้น ห้ามมีคำอธิบายเพิ่มเติม:\n\n" + convo
    )
