...
def add_fact(mem, text, category=None):
    ...
    # ว่าหมวดที่โมเดลตั้งเองควรเป็น single-value หรือ multi-value เดาผิดแล้วลบของจริงทิ้ง
    # แย่กว่าเก็บซ้ำ (บทเรียนจากบั๊ก [งาน]"เล่นเกมมา 3 ปีแล้ว" ที่ลบอาชีพจริงทิ้ง)
...
def should_try_extract(text: str) -> bool:
    ...
    ⚠️ เดิมกรองด้วย SELF_REFERENCE_HINTS (คำใบ้ 20 คำ) — วัดกับชุดทดสอบ 80 เคสพบว่า
    **ตัดทิ้ง 33/80 เคสก่อนถึงโมเดลเลย** ("แพ้กุ้ง" "ขับรถไม่เป็น" "นับถือพุทธ"
    "ตื่นตี 5 ทุกวัน" ไม่มีคำใบ้สักคำ) ทั้งที่เป็นข้อมูลที่ควรจำทั้งนั้น

    ตรงกับที่ mem0 สรุปไว้: *"The LLM is a more capable filter than any pre-computed
    retrieval structure, at the cost of a few extra tokens"* — ลิสต์คำใบ้คือ
    pre-computed filter ที่ไม่มีวันครบ (บทเรียนซ้ำจาก _HAIR_NEXT / _CHANGE_SIGNALS)

    เหลือไว้แค่กรอง "ข้อความที่ไม่มีเนื้อหาให้สกัดแน่ๆ" เพื่อไม่เปลือง LLM call ทุกข้อความ:
    สั้นมาก (< 6 ตัว) หรือเป็นคำตอบรับ/ทักทายล้วน — ที่เหลือปล่อยให้โมเดลตัดสินเอง
    (โมเดลคืน [] ได้อยู่แล้วถ้าไม่มีอะไรน่าจำ)
    """
    t = text.strip()
    if len(t) < 6:                       # สั้นเกินไป (ทักทาย/คำเดียว) ข้าม
        return False
    if is_social_pleasantry(t):          # "ขอบคุณมากค่ะ" / "โอเคครับ" — ไม่มีอะไรให้จำ
        return False
    if _looks_like_question(t):          # คำถามไม่ใช่การบอกข้อมูลตัวเอง
        return False
    if is_recall_question(t):            # "จำได้ไหมว่า..." = ถามความจำ ไม่ใช่บอกข้อมูลใหม่
        return False
    if ends_like_question(t):            # ลงท้ายแบบคำถาม = ถาม ไม่ใช่บอกข้อมูลตัวเอง
        return False
    return True


# คำที่บ่งว่าข้อความเป็น *คำถาม* ไม่ใช่การบอกข้อมูลเกี่ยวกับตัวเอง
#
# เหลือด่านนี้ไว้หลังตัดลิสต์คำใบ้ทิ้ง — ไม่ใช่เพื่อกันข้อมูลหลุด แต่เพื่อไม่เปลือง LLM call
# กับข้อความที่ *แน่นอนว่าไม่มีอะไรให้จำ* ("อากาศวันนี้เป็นยังไง" "มีวิธีไหมบ้าง")
#
# ⚠️ ต่างจากลิสต์คำใบ้เดิมตรงที่นี่เป็น **blacklist ของรูปแบบที่ชัดเจน** ไม่ใช่ whitelist
# ที่ต้องเดาให้ครบว่าคนจะพูดอะไรบ้าง — พลาดทางนี้แค่เปลือง call ไม่ได้ทำข้อมูลหาย
_QUESTION_MARKERS = ("ไหม", "หรือเปล่า", "รึเปล่า", "ยังไง", "อย่างไร", "เท่าไหร่", "เท่าไร",
                     "ทำไม", "อะไรบ้าง", "ที่ไหน", "เมื่อไหร่", "ใครบ้าง", "?")


# คำถามที่ผู้ใช้ถามว่า "จำได้ไหม/เคยคุยไหม" — เป็นการ *ถาม* ความจำ ไม่ใช่การ *บอก* ข้อมูล
#
# 🚨 เจอตอนคุยจริงหลัง restart: "จำได้ไหมว่าผมทำงานอะไร" ถูกเก็บเป็น fact
#     [กิจวัตร] จำได้ไหมว่าผมทำงานอะไร
# guard ที่มีอยู่ไม่จับ เพราะ _looks_like_question ปล่อยผ่านทุกประโยคที่มีสรรพนาม
# (กฎนั้นตั้งไว้กันเคส "ผมควรกินยาตอนไหนดี" ที่มีข้อมูลตัวเองปนอยู่จริง)
#
# อันตรายเพราะสะสม — ทุกครั้งที่ผู้ใช้ถามเรื่องความจำ จะได้ fact ขยะเพิ่มอีกอัน
# เป็นปัญหาเดียวกับ meta tag ที่แก้ไปแล้วฝั่ง summary (is_meta_summary_tag)
#
# ⚠️ ต้องจับให้แคบ: "ผมเคยไปญี่ปุ่นมาแล้ว" มีคำว่า "เคย" แต่เป็นการ *บอก* ประสบการณ์
# จึงจับเฉพาะรูปแบบ "ถามความจำ" ที่ชัดเจน ไม่ใช่คำว่า "เคย" ลอยๆ
_RECALL_QUESTION_RE = re.compile(
    r"(จำได้ไหม|จำ.{0,6}ได้ไหม|จำได้หรือเปล่า|จำได้รึเปล่า|ยังจำ.{0,10}ได้ไหม|"
    r"เคย.{0,12}(ไหม|หรือเปล่า|รึเปล่า)|เราเคยคุย|เคยคุยกัน|เคยบอก.{0,6}ไหม|"
    r"เคยเล่า.{0,16}ไหม|จำ.{0,4}ได้บ้าง|จำอะไรได้)")


def is_recall_question(text: str) -> bool:
    """ผู้ใช้กำลัง *ถาม* ความจำ (ไม่ใช่ *บอก* ข้อมูลใหม่) หรือเปล่า"""
    return bool(_RECALL_QUESTION_RE.search(text))


# คำถามภาษาไทยวางคำถามไว้ **ท้ายประโยค** เกือบเสมอ ("...ไหม" "...ยังไงดี" "...เหรอ")
# ส่วนประโยคบอกเล่าไม่ลงท้ายแบบนี้ → ใช้ *ตำแหน่ง* แยกได้ โดยไม่ต้องเดาระยะห่างในสตริง
#
# 🚨 ที่มา: fact ขยะจากการคุยจริง 2 รอบ
...
#
# guard เดิมไม่จับเพราะ:
#   - is_recall_question ใช้ `เคยเล่า.{0,16}ไหม` แต่ช่องว่างจริง 19 ตัว
#     การไล่ขยายตัวเลขคือ whack-a-mole (MEMORY_EXPERIMENTS §4 เตือนไว้แล้ว)
#   - _looks_like_question ปล่อยผ่านทุกประโยคที่มีสรรพนาม (กฎกันเคส "ผมควรกินยาตอนไหนดี")
#
# ⚠️ ต้องตัดคำลงท้ายสุภาพออกก่อน ("...ไหมคะ" "...เหรอครับ") ไม่งั้นหางไม่ตรง
# ⚠️ "ดี" เดี่ยวๆ ใช้ไม่ได้ — "สนุกดี" "อร่อยดี" เป็นคำขยาย ไม่ใช่คำถาม
# (เจอจริง: "เมื่อวานพาลูกไปสวนสาธารณะมา สนุกดี" ถูกกรองทิ้งทั้งที่เป็นข้อมูล)
# จึงรับเฉพาะรูปที่เป็นคำถามชัดเจน เช่น "ยังไงดี" "แบบไหนดี" ซึ่งอยู่ในลิสต์อยู่แล้ว
_QUESTION_TAILS = (
    "ไหม", "มั้ย", "เหรอ", "หรอ", "รึเปล่า", "หรือเปล่า", "ยังไง", "ยังไงดี",
    "อย่างไร", "อะไร", "ไหน", "ไหนดี", "เท่าไหร่", "เท่าไร", "ทำไม", "ใคร",
    "เมื่อไหร่", "บ้าง",
)
_POLITE_TAIL_RE = re.compile(r"[?？ๆ\s]*(ครับ|ค่ะ|คะ|นะ|น่ะ|จ๊ะ|จ้า|ฮะ|ฮ่ะ|เนอะ|น้า)*[?？\s]*$")


def ends_like_question(text: str) -> bool:
    """ประโยคนี้ลงท้ายแบบคำถามไหม — ใช้ตำแหน่งท้ายประโยคแทนการนับระยะห่าง"""
    s = _POLITE_TAIL_RE.sub("", text.strip())
    return any(s.endswith(t) for t in _QUESTION_TAILS)


def _looks_like_question(text: str) -> bool:
    """ข้อความนี้เป็นคำถามล้วนๆ ไหม (ไม่มีการบอกข้อมูลตัวเองปนอยู่)

    ถ้ามีสรรพนามบุรุษที่ 1 ปนอยู่ ถือว่าอาจมีข้อมูลตัวเอง → ปล่อยให้โมเดลตัดสิน
    ("ผมควรกินยาตอนไหนดี" มีข้อมูลว่าผู้ใช้กินยา แม้จะอยู่ในรูปคำถาม)
    """
    if any(p in text for p in ("ผม", "ฉัน", "หนู", "เรา", "ดิฉัน", "กระผม")):
        return False
    return any(q in text for q in _QUESTION_MARKERS)


def build_extract_prompt(user_message: str) -> str:
    """สร้าง prompt สั่งโมเดลสกัดข้อเท็จจริงถาวรเกี่ยวกับผู้ใช้ — **หมวดเปิด** ตั้งชื่อเองได้

    ⚠️ เดิมบังคับเลือกจาก closed set 7 หมวด (ชื่อ/ที่อยู่/งาน/ความชอบ/ของที่มี/
    เรื่องที่สนใจ/หัวข้อสนทนา) วัดกับชุดทดสอบ 80 เคส 10 กลุ่มข้อมูลแล้วพบว่า:
        กลุ่มที่ *มีหมวดรองรับ*     จำได้ 5/8  = 62%
        กลุ่มที่ *ไม่มีหมวดรองรับ*  จำได้ 6/72 =  8%   ← ต่างกัน 8 เท่า
    ข้อมูลอย่าง แพ้กุ้ง / มีลูก 2 คน / วันเกิด / กลัวความสูง / นับถือพุทธ / ตื่นตี 5
    ไม่มีหมวดให้ใส่ โมเดลจึงคืน [] ทิ้งข้อมูลไปเงียบๆ

    เปิดเป็น open schema แล้ววัดซ้ำบนชุดเดิม: 11/80 (14%) → 45/80 (56%)
    ช่วงความเชื่อมั่นไม่ซ้อนทับ ([8-23%] vs [45-67%]) = ดีขึ้นจริงทางสถิติ
    และโมเดลตั้งหมวดเองได้ใกล้เคียงกับที่คนออกแบบไว้ (สุขภาพ/ครอบครัว/กิจวัตร/ทักษะ/…)

    ตรงกับงานวิจัย: arXiv 2604.11610 วัดได้ว่า self-evolving schema ชนะ fixed taxonomy
    และ "detailed taxonomy ไม่การันตีว่าสกัดได้ดีขึ้น" — ปัญหาคือ *การมี* fixed taxonomy
    ไม่ใช่ taxonomy ยังไม่ครบ (ซึ่งเป็นทางที่เกือบเลือกไปเพิ่มหมวด)

    กฎเรื่องปฏิเสธเพิ่มมาเพราะ baseline วัดได้ว่าจำประโยคปฏิเสธไม่ได้เลย 0/12
    ("กินเผ็ดไม่ได้" "ขับรถไม่เป็น" "ไม่กินเนื้อวัว") ทั้งที่เป็นข้อมูลที่ควรจำที่สุด
    """
    return (
...
# JSON schema บังคับให้โมเดลตอบเป็น **array** ของ fact — ส่งเป็น `format` ให้ Ollama
#
# 🚨 ทำไมต้องมี: `format: "json"` เฉยๆ (constrained decoding แบบหลวม) ทำให้โมเดลคืน
# object เดี่ยว มันจึงหยุดหลังสกัดได้ fact แรก ประโยคที่มี 2 ข้อมูลจึงเก็บครบแค่ 1/5:
...
# ใช้กับ agent memory โดยตรง ได้ F1 20.97 -> 42.50 บน multi-hop
# ⚠️ แต่ arXiv 2603.28005 เตือนว่า decompose ไม่ได้ดีเสมอ: ช่วยงานง่าย แต่ทำงานซับซ้อน
# แย่ลง และ "strips away essential contextual information"
#
# จึงเลือกทางที่ **ไม่ต้อง decompose เพิ่มรอบ** — แค่บอก schema ว่าตอบได้หลายอัน
# ไม่เพิ่ม LLM call เลย วัดแล้ว: ประโยคหลายข้อมูลครบ 5/5 · ชุด 80 เคส 79/80 · แต่งเรื่อง 0%
EXTRACT_JSON_SCHEMA = {
    "type": "object",
    "properties": {
        "facts": {
...
def parse_extracted_facts(model_output: str) -> list:
    """แปลงผลที่โมเดลตอบ (JSON array หรือ object เดี่ยว) เป็น list ของ dict

    ทนทานต่อกรณีโมเดลใส่ข้อความเกินมา — ดึงเฉพาะส่วน JSON ออกมา parse
    - รองรับทั้ง `[{...}, {...}]` และ `{...}` เดี่ยว
    - รองรับกรณีโมเดลเผลอตอบเป็น list ของสตริงล้วน (รูปแบบเก่า) ด้วย
    คืน list ของ {"category": str|None, "text": str}

    ⚠️ ที่ต้องรับ object เดี่ยวด้วย: การใช้ `format: json` (constrained decoding) ทำให้
    โมเดลคืน object เดี่ยวแทน array — จำเป็นกับ qwen3:4b ซึ่งเป็นโมเดลที่สกัดแม่นที่สุด
    (89%) แต่ถ้าไม่บังคับ format มันจะ "คิดออกเสียง" ก่อนตอบเสมอ ทำให้ช้า 13-30s
    ต่อ call (เขียนการคิดลง content 1,022 tokens) — บังคับ format:json แล้วเหลือ 0.8s
    """
    ...
    # หา array ก่อน (ได้ข้อมูลครบกว่า) ไม่เจอค่อยหา object เดี่ยว
    ...
    for it in items:
        ...
        if not (2 <= len(text) <= 60):   # ยาวเกิน 60 ตัว = น่าจะไม่ใช่ fact สั้นๆ
            continue
        # หมวดเปิด — เก็บชื่อหมวดที่โมเดลตั้งเองไว้ ไม่ทิ้งเป็น None เหมือนเดิม
        # (เดิมทิ้งทุกหมวดนอก closed set 7 หมวด = ข้อมูล 9 ใน 10 กลุ่มจำไม่ได้เลย)
        # หมวดยาวเกิน 20 ตัว = โมเดลเขียนประโยคมาแทนชื่อหมวด → ไม่รับ
        if not isinstance(category, str) or not (1 <= len(category.strip()) <= 20):
            category = None
        else:
            category = category.strip()
...
def build_summary_prompt(pairs: list) -> str:
    ...
    return (
        ...
        '- "user_pref:<สิ่งที่ผู้ใช้ชอบ/ไม่ชอบ *จริงๆ*>"\n'
        '- "user_fact:<ข้อเท็จจริงของผู้ใช้>"\n'
        '- "user_ask:<สิ่งที่ผู้ใช้ขอ/อยากได้ในบทนี้>"\n'
        '- "me_pref:<สิ่งที่รอสเต้เองชอบ/ไม่ชอบ *จริงๆ*>"\n'
        '- "me_fact:<สิ่งที่รอสเต้ทำหรือเป็น>"\n'
        '- "me_suggest:<สิ่งที่รอสเต้แนะนำ/เสนอให้ผู้ใช้>"\n'
        "กฎ:\n"
        ...
        # ⚠️ กฎ 2 ข้อล่างนี้เพิ่มเพราะวัดเจอว่า tag 19% ติดผิดชนิด (28/144 ในความจำจริง)
        # ต้นเหตุ: schema เดิมมีแค่ pref/fact — "ผู้ใช้ขอวิธีจัดการเวลา" กับ "รอสเต้แนะนำ
        # Pomodoro" ไม่มีช่องให้ลง โมเดลจึงยัดลง pref ทั้งคู่ กลายเป็น "ผู้ใช้ชอบงานหนัก"
        # และ "รอสเต้ชอบ Pomodoro" ซึ่งผิดทั้งคู่ (ทำซ้ำได้ 2/3 เคสที่ temperature 0)
        "- \"ขอ/อยากได้/ต้องการ\" = user_ask ไม่ใช่ user_pref (การขอ ≠ การชอบ)\n"
        "- \"รอสเต้แนะนำ/เสนอ X\" = me_suggest ไม่ใช่ me_pref (การแนะนำ ≠ รอสเต้ชอบเอง)\n"
        # ⚠️ กฎนี้เพิ่มหลังคุยจริง 14 คู่ — พบว่าการเพิ่ม user_ask/me_suggest ทำให้โมเดล
        # ระวังเกินไป แล้วย้าย "ชอบ/ไม่ชอบ" จริงๆ ไปลง fact แทน
        #   "ผมชอบอ่านนิยายสืบสวน" -> user_fact:ชอบอ่านนิยายสืบสวน  (เดิมได้ user_pref ถูก)
        # ซึ่งทำให้คำถามความชอบ ("ผมชอบอ่านแนวไหน") หาไม่เจอ เพราะ B1 กรอง kind='pref'
        "- \"ชอบ/ไม่ชอบ/ชื่นชอบ\" = _pref เสมอ (อย่าเอาไปลง _fact)\n"
...
_OWNER_TAG_RE = re.compile(
    r"(user_pref|user_fact|user_ask|me_pref|me_fact|me_suggest)\s*:")
...
def split_owner_tags(text: str) -> dict:
    ...
    out = {"summary": head, "user": [], "me": [],
           "user_pref": [], "user_fact": [], "user_ask": [],
           "me_pref": [], "me_fact": [], "me_suggest": []}
    for i, mk in enumerate(marks):
...
def filter_by_owner(texts, whose: str, kind: str = None) -> list:
    ...
    for t in texts:
        ...
        if kind == "pref":
            # ⚠️ วัดจากบทคุยจริง: 4B ติด "ชอบอ่านนิยายสืบสวน" เป็น _fact ไม่ใช่ _pref
            # (ยิ่งบทยาวหลายเรื่องยิ่งพลาดบ่อย) -> คำถามความชอบได้ [] ทั้งที่ข้อมูลอยู่ครบ
            #
            # แก้ที่ฝั่ง *อ่าน* ไม่ใช่ไปดัด prompt ให้โมเดล tag ถูก เพราะฝั่งอ่านคุมได้แน่นอน
            # ส่วนโมเดลเป็นของที่บังคับไม่ได้ 100% (กฎใน prompt ช่วยบทสั้น แต่ไม่การันตีบทยาว)
            # เอาเฉพาะ _fact ที่ขึ้นต้นด้วยคำบอกความชอบจริงๆ — ไม่ใช่ _fact ทั้งหมด
            # ("แนะนำร้านให้" ต้องไม่หลุดเข้ามา ไม่งั้นย้อนกลับไปเป็นบั๊ก B1 เดิม)
            own = own + [v for v in (parts.get(f"{whose}_fact") or [])
                         if v.startswith(_LIKE_PREFIXES)]
        if not own:
            # ไม่มีของชนิดที่ขอ — ตัดทิ้ง (ตอบว่าจำไม่ได้ดีกว่าเอาของอีกฝั่งมาตอบ)
            #
            # ⚠️ เคยลอง "กู้" บรรทัดที่มี me_fact กลับมาตอนถามฝั่งผู้ใช้ เพื่อกันบอทแต่งคำตอบ
            # (เคส "ผมใช้ Pomodoro อยู่ใช่ไหม") — **ถอยออกแล้ว** เพราะทำให้การแยกเจ้าของพัง
            # ซึ่งเป็นหลักประกันที่วัดมาแล้วว่าลดการสลับเจ้าของจาก 29% -> 0% (วิธี F / B1)
            # และวัดจริงหลังใส่: attribution error แย่ลง 50% -> 67% ไม่ใช่ดีขึ้น
            # -> การเอาของฝั่งรอสเต้มาไว้ในคำตอบฝั่งผู้ใช้ ทำให้โมเดล *สับสนมากขึ้น* ไม่ใช่น้อยลง
            continue
        out.append(f"{parts['summary']} — {label}: {_with_provenance(parts, key, own)}")
    return out


# ── provenance: แยก "สิ่งที่ยืนยันแล้ว" ออกจาก "สิ่งที่รอสเต้แค่เสนอไป" ──────────

# me_fact = สิ่งที่รอสเต้ *ทำ* ซึ่งในทางปฏิบัติเกือบทั้งหมดคือ "เคยแนะนำอะไรบางอย่าง"
# วัดกับความจำจริง: me_fact ทุกอันขึ้นต้นด้วย "แนะนำ" (แนะนำวิธีจัดการงาน 4x, แนะนำเทคนิค
# Pomodoro 2x, ...) แต่เดิมแสดงผลเป็น "— รอสเต้: แนะนำเทคนิค Pomodoro" ซึ่งหน้าตา
# **เหมือนกันเป๊ะ** กับข้อเท็จจริงที่ผู้ใช้ยืนยัน โมเดลจึงตอบราวกับผู้ใช้ใช้ Pomodoro อยู่จริง
#
# วัด baseline ได้ attribution error 50% (9/18) และระบุเจ้าของถูก 0/18
# — ตรงกับที่ audit ข้อ 2 เตือนไว้ว่าเป็นช่องว่างใหญ่สุดของ write path
# ดู tools/bench_provenance.py
_PROVENANCE_HINT = "(รอสเต้เป็นคนเสนอ ยังไม่ยืนยันว่าผู้ใช้ทำตาม)"

# ฝั่งผู้ใช้ก็มีปัญหาเดียวกันแต่คนละรูป: "ต้องการ/อยากได้/ขอ X" = ผู้ใช้ **ขอ** X
# ไม่ใช่ผู้ใช้ **ทำ/ใช้** X อยู่แล้ว
# วัดจริง: ถาม "ผมเคยใช้คำกล่าวขอบคุณแบบไหน" -> ได้ context "ผู้ใช้: ต้องการคำกล่าวขอบคุณ"
# -> โมเดลตอบว่า "คุณเคยใช้คำกล่าวขอบคุณแบบเป็นทางการค่ะ อย่างเช่น ..." แล้วแต่งตัวอย่างเอง
# ทั้งที่ความจำบอกแค่ว่าผู้ใช้ *ขอ* มา และตัวอย่างพวกนั้นเป็นของที่รอสเต้แต่งให้
_WANT_PREFIXES = ("ต้องการ", "อยากได้", "อยากรู้", "ขอคำ", "ขอตัวอย่าง", "สนใจ")

# คำที่บอกว่า _fact อันนี้จริงๆ แล้วเป็น "ความชอบ" — ใช้กู้ตอนถามความชอบ
# (4B ติด tag ผิดชนิดเป็นครั้งคราว โดยเฉพาะบทยาว — ดู filter_by_owner)
_LIKE_PREFIXES = ("ชอบ", "ไม่ชอบ", "ชื่นชอบ", "รัก", "เกลียด", "โปรด")
_WANT_HINT = "(ผู้ใช้ขอมา ไม่ใช่สิ่งที่ผู้ใช้ทำอยู่)"


def _with_provenance(parts: dict, key: str, own: list) -> str:
    """ต่อท้ายที่มาให้เฉพาะฝั่งรอสเต้ชนิด fact — ฝั่งผู้ใช้และ pref ไม่แตะ

    ทำที่ชั้น *แสดงผล* ไม่ใช่ชั้นเก็บ เพราะ:
      - ไม่ต้อง migrate ความจำเดิม 55 อัน (และของผู้ใช้อื่นอีก 3 คน)
      - ไม่เพิ่มภาระ write path ที่วัดแล้วว่าเปราะ (สกัดพลาด = เสียถาวร)
      - ถ้าไม่ดีก็ถอยได้ทันทีโดยข้อมูลไม่เสียหาย

    ⚠️ key='me' (kind=None) คือเส้นที่ production ใช้จริง — รวม me_fact กับ me_pref ไว้ด้วยกัน
       จึงต้องมาร์กเป็นรายอัน ไม่ใช่ต่อท้ายทั้งก้อน ไม่งั้น me_pref ("ชอบอ่านหนังสือ")
       จะถูกป้ายว่าเป็นข้อเสนอไปด้วย ซึ่งผิด — มันคือความชอบของรอสเต้เองจริงๆ
    """
    # tag ชนิดใหม่ (user_ask / me_suggest) บอกที่มาในตัวเองอยู่แล้ว จึงติดป้ายตรงๆ
    # ส่วน heuristic ตามคำขึ้นต้นยังต้องเก็บไว้ เพราะความจำเก่า 55 อันไม่มี tag ใหม่
    # (เขียนไปแล้วด้วย schema เดิม — ไม่ migrate ย้อนหลัง ดูเหตุผลใน _with_provenance)
    asks = set(parts.get("user_ask") or [])
    suggests = set(parts.get("me_suggest") or [])

    def mark(v: str, side: str) -> str:
        if side == "user":
            return f"{v} {_WANT_HINT}" if v in asks or v.startswith(_WANT_PREFIXES) else v
        if v in suggests or v in set(parts.get("me_fact") or []):
            return f"{v} {_PROVENANCE_HINT}"
        return v

    side = "user" if key.startswith("user") else "me"
    if key in ("me_fact", "me_suggest"):
        return f"{', '.join(own)} {_PROVENANCE_HINT}"
    if key == "user_ask":
        return f"{', '.join(own)} {_WANT_HINT}"
    return ", ".join(mark(v, side) for v in own)
...
def _word_weight(word: str, texts: list) -> float:
    """น้ำหนักคำแบบ IDF ย่อๆ — คำที่โผล่ในไม่กี่ summary มีค่ามากกว่าคำที่โผล่แทบทุกอัน

    ทำไมไม่ใช้ BM25/IDF เต็มสูตร: ชุดข้อมูลเล็กมาก (55 อัน) สูตรเต็มให้ค่าแกว่งแรง
    และเราต้องการแค่ "อย่าให้คำทั่วไปกลบคำชี้ขาด" ไม่ได้ต้องการจัดอันดับละเอียด

    ค่า: คำที่อยู่ใน <=20% ของ summary ได้ 3.0 · <=50% ได้ 1.5 · นอกนั้น 1.0
    (ขั้นบันไดหยาบๆ ตั้งใจ — กันไม่ให้ผลแกว่งตามจำนวน summary ที่โตขึ้นเรื่อยๆ)
    """
    if not texts:
        return 1.0
    hits = sum(1 for t in texts if word in t)
    if hits == 0:
        return 1.0
    ratio = hits / len(texts)
    if ratio <= 0.2:
        return 3.0
    return 1.5 if ratio <= 0.5 else 1.0
...
def recall_summaries(mem, user_message: str, top_k: int = 5, now=None) -> list:
    ...
    for text, when in zip(texts, dates):
        ...
        # คำที่ "หายาก" ต้องมีน้ำหนักกว่าคำทั่วไป — เดิมนับเท่ากันหมด ทำให้ 'Pomodoro'
        # (คำชี้ขาด อยู่ใน summary เดียว) มีค่าเท่ากับ 'ผม' (อยู่แทบทุกอัน)
        # บรรทัดที่มีคำชี้ขาดจึงแพ้บรรทัดที่บังเอิญมีคำทั่วไปหลายคำ
        score = sum(_word_weight(w, texts) for w in words if w in hay)
        if score > 0:
