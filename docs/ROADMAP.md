...
- [x] **ลอง VoxCPM2 เป็น TTS หลัก แล้ว *ย้อนกลับ* มาใช้ F5** (`VOXCPM_ENABLED = False`) — เสียงเป็นธรรมชาติ
  กว่า F5 จริง แต่รากปัญหาคือ **RTF > 1** (เจนช้ากว่าเสียงที่ได้) ซึ่งบังคับให้เลือกทางที่แย่ทั้งคู่:
  ซอย segment → ช่องว่าง 12-16s ทุกจุด + เสียงเปลี่ยนตรงรอยต่อ / ไม่ซอย → รอ 36.8s ก่อนได้ยินอะไรเลย
  และช้ากว่าการซอยอีก (RTF แย่ลงตามความยาว: 1.55 ที่ 123c → 1.73 ที่ 266c)
  - เทียบกับ F5 ที่ RTF 0.65 (เจนทันเล่น) — prefetch กลบช่องว่างได้หมด
  - เก็บโค้ด + `voxcpm_worker.py` ไว้ครบ ตั้ง `True` เปิดกลับได้ทันทีถ้ามีโมเดลที่ RTF < 1
- [x] **ข้าม RVC สำหรับ TTS ปกติ** (`F5_THEN_RVC = False`) — F5 → pitch 108% ล้วน ผู้ใช้ฟังเทียบแล้วเลือก
  ทางนี้ ลด latency ลงหนึ่งขั้น (RVC ยังจำเป็นสำหรับ karaoke จึงเก็บ worker ไว้ทั้งหมด)
- [x] **fade รอยต่อระหว่าง segment** — เดิมต่อแบบตัดสนิททำให้มีเสียงคลิก/ป๊อปตรงรอยต่อ
  - เส้น streaming (บอทใช้จริง): Discord เล่นทีละไฟล์ → **crossfade แท้ทำไม่ได้** เพราะคลื่นสองก้อนไม่เคย
    เล่นซ้อนกัน ใช้ `_apply_edge_fade()` fade หัว/ท้าย 15ms ต่อ segment แทน
  - เส้นไฟล์เดียว (`_concat_wavs`): รวมเป็น array เดียวได้ → crossfade แท้ 150ms
  - `SEG_EDGE_FADE_MS` / `SEG_CROSSFADE_MS` ตั้ง 0 กลับพฤติกรรมเดิมได้ทันที
  - ทดสอบจริง: 20c/1ส่วน, 184c/3ส่วน, 735c/15ส่วน, 569c/9ส่วน — ผ่านทุกรอยต่อ ไม่กระทบ prefetch
  - ⚠️ แก้ได้แค่เสียงคลิก **ไม่ได้แก้โทนกระโดด** ที่มาจาก segment เจนแยกกันโดยไม่รู้บริบทก้อนก่อนหน้า
  - ⚠️ เคยลอง chain ref audio (ใช้เสียง segment ก่อนหน้าเป็น ref ตัวถัดไป) เพื่อแก้โทนกระโดด — คุณภาพ
    เสื่อมสะสมทีละ segment (error compounding) เลิกใช้ กลับมา fix ที่ `F5_REF_AUDIO` เดิมเสมอ
...
- [ ] **ทับศัพท์คำอังกฤษก่อนส่งเข้า F5** — ยืนยันแล้วว่า `th_to_g2p` แปลงเฉพาะอักษรไทย
  คำอังกฤษหลุดไปหาโมเดลดิบๆ (ดู Known Issues หมวดเสียงพูด) ยังไม่ได้เลือกแนวทาง:
  พจนานุกรมคำที่เจอบ่อย / `pythainlp.transliterate` อัตโนมัติ / ผสมสองอย่าง
- [ ] **`use_ipa` ใน `tools/test_crossfade.py`** — สคริปต์ทดสอบฮาร์ดโค้ด `use_ipa=True`
  ขณะที่ `TTS.infer` คำนวณจาก `model_type` เป็นผู้ต้องสงสัยที่เหลือของอาการเสียงมั่ว
  ที่เจอตอนเรียก `infer_batch_process` ตรงๆ (pipeline จริงไม่ได้รับผลกระทบ — ใช้ `tts.infer`)
...
- **cold load ~13–36s** (วัดจาก log จริงหลายรอบ แกว่งตาม disk cache) — บอทรอ F5 worker
  พร้อมก่อนตอบด้วยเสียงได้ (ตอบแชตได้ก่อน warm เสร็จ)
- **คำภาษาอังกฤษยังไม่ทับศัพท์** — `th_to_g2p` ของ f5_tts_th แปลงเฉพาะอักษรไทยเป็น IPA
  ส่วนคำอังกฤษถูกส่งผ่านดิบๆ (`game` → `game.` แต่ `เกม` → `keːm.`) โมเดลจึงได้ input
  ปนกันระหว่าง IPA กับตัวอักษรละติน ทั้งที่เทรนมาให้อ่าน IPA → อ่านเพี้ยน/มั่วได้
  ตอนนี้ `f5_preprocess.py` มีแค่กฎเจาะจง (B20/E85/NGV) ยังไม่มีกฎทั่วไป — ยังไม่แก้
- **โทน/จังหวะกระโดดตรงรอยต่อ segment** — แต่ละก้อนเจนแยกกันจาก ref เดียวโดยไม่รู้บริบท
  ก้อนก่อนหน้า edge fade 15ms แก้ได้แค่เสียงคลิก ไม่ได้แก้เรื่องนี้ (ข้อจำกัดสถาปัตยกรรม)
...
| RVC worker ไม่โหลด / CUDA error | Python version หรือ torch CUDA mismatch | RVC ต้องรันใน `rvc_venv` (Python 3.10 + torch CUDA ที่ตรงกับการ์ด — เครื่อง dev ใช้ cu128) แยกจาก main env |
...
- **VLM (Vision Model) — เดิมสรุปว่า "Qwen3-VL-8B ไม่พอดีแน่นอน" (3 ก.ค. 2569, ตอนอยู่โน้ตบุ๊ค 4GB)**
  → **ข้อจำกัดเรื่อง VRAM หมดไปแล้ว** แต่ **ยังไม่ได้ทำฟีเจอร์** — แยกสองเรื่องนี้ให้ชัด

  **สิ่งที่วัดแล้วจริง (6 ส.ค. 2569) — แค่ระดับ "โมเดลรันบนเครื่องนี้ได้":**
  - `qwen3-vl:8b` กิน **5.79GB** — ยิงผ่าน Ollama API ตรงๆ 2 คำถาม ด้วย**รูปที่สร้างขึ้นเองเพื่อเทส**
    (กล่อง error จำลอง วาดด้วย System.Drawing) อ่านถูกทั้งชนิด error, เลขบรรทัด (412), ปุ่มในภาพ
    ตอบภาษาไทยได้ 4.5-13.9s ต่อภาพ
  ...
  **สิ่งที่ยังไม่ได้ทดสอบเลย (= งานจริงเกือบทั้งหมด):**
  - ยังไม่ได้แตะโค้ดในโปรเจกต์สักบรรทัด — บอทยังไม่รู้จัก vision
  - ยังไม่เคยลองกับ**รูปจริง** (สกรีนช็อตจริง, รูปถ่าย, ภาพเบลอ, ภาพที่มีข้อความไทยในภาพ)
    — รูปที่เทสเป็นตัวอักษรคมชัดบนพื้นทึบ ซึ่งง่ายกว่าของจริงมาก
  - ยังไม่ได้ทดสอบเส้นทาง Discord attachment → บอท → VLM
  - ยังไม่ได้ทดสอบร่วมกับ persona / ความจำ / `select_tools()`
  - **"รอสเต้มองเห็นจอ" (แคปหน้าจอเอง) ยังไม่ได้ทดสอบเลย** — คนละเรื่องกับ "ผู้ใช้แนบรูปมาให้ดู"
    ต้องมีขั้นแคปหน้าจอ + ตัดสินใจว่าจะแคปเมื่อไหร่ ซึ่งยังไม่มีอะไรรองรับ
  - ⚠️ ประเด็นความเป็นส่วนตัว: ถ้าให้แคปหน้าจอได้ ต้องคิดเรื่องขอบเขต/การขออนุญาตก่อนออกแบบ

  สรุป: **"โมเดลทำงานได้" ≠ "ฟีเจอร์พร้อม"** — ตอนนี้อยู่ระดับแรกเท่านั้น

- **"ดูวิดีโอ/ดูจอสด" — ทดสอบแล้ว 6 ส.ค. 2569 (ยังเป็นระดับโมเดล ไม่ใช่ฟีเจอร์)**

  ⚠️ **ไม่มี LLM ตัวไหน "ดูวิดีโอ" ได้จริง** — ทุกตัวรับได้แค่ภาพนิ่ง สิ่งที่เรียกว่าดูวิดีโอได้คือ
  **สุ่มเฟรมออกมาเป็นภาพนิ่งหลายใบแล้วส่งเข้าไปพร้อมกัน** เราเป็นคนเลือกเองว่าจะเก็บกี่เฟรม ถี่แค่ไหน

  **ผลทดสอบ (คลิปจำลอง 8 เฟรม: บอลวิ่งซ้าย→ขวา + ไฟเขียว→แดงที่เฟรม 5):**
  - ✅ **เห็นความเปลี่ยนแปลงข้ามเฟรมได้จริง** — ตอบทิศทางถูกทุกครั้ง และจับจุดที่ไฟเปลี่ยนได้
    (4 เฟรม → ตอบ "เฟรม 5/8" ตรงเป๊ะ)
  - ⚠️ **ต้องขยาย `num_ctx` เอง** — ค่า default ของ Ollama คือ **4096** (ไม่ใช่ 262144 ที่โมเดลรองรับ)
    ส่ง 8 เฟรมแล้ว error ทันที `exceed_context_size_error` ต้องตั้ง `options.num_ctx` เอง
  - ⚠️ **ถ้า ctx เล็กเกินไป มันจะ "ยำ" ทุกเฟรมรวมเป็นภาพเดียว** — ทดสอบครั้งแรกที่ ctx 4096
    ตอบว่าเห็น "วงกลมเขียว 3 วง" (= ลูกบอล 3 ตำแหน่งทับกัน) และ hallucinate ALERT
    เข้าไปในเฟรมที่ยังเขียวอยู่ **ผิดแบบดูเผินๆ เหมือนถูก** ซึ่งอันตรายกว่าตอบว่าไม่รู้

  **ตัวเลขจริง (รูป 640×360, รอบสอง = warm):**

  | เฟรม | num_ctx | tokens | latency | VRAM |
  |------|---------|--------|---------|------|
  | 1 | 4096 | 1,098 | **4.1s** | 5.79 GB |
  | 2 | 8192 | 1,098 | 4.0s | 6.54 GB |
  | 4 | 8192 | 2,175 | 6.2s | 6.54 GB |
  | 8 | 16384 | 4,329 | **7.0s** | 7.76 GB |

  - **~540 tokens/เฟรม** → ค่านี้ใช้ประมาณ ctx ที่ต้องใช้ได้เลย
  - `num_ctx` **จอง VRAM ล่วงหน้า** — ตั้ง 65536 ไว้ VRAM พุ่งไป **14.07GB** (จาก 16.31GB)
    ต้องตั้งพอดีกับงาน ไม่ใช่ตั้งสูงไว้ก่อน ไม่งั้นแย่ง VRAM ของ qwen3:8b + RVC
  - ส่งรูป 1280×720 ได้ token เท่ากับ 640×360 เป๊ะ → **Ollama ย่อรูปให้เองอยู่แล้ว**
    ส่งรูปใหญ่ไปไม่ได้ช่วยอะไร เปลืองแบนด์วิดท์เปล่า

  **แปลว่า "ดูจอสด" แบบ realtime ไม่ได้** — เร็วสุด 4s/ภาพ (ยังไม่รวมเวลาแคปหน้าจอ + เวลาที่
  qwen3:8b ต้องเอาไปตอบต่อ) เทียบกับ 30fps ที่คนเรียกว่าวิดีโอจริงๆ ต่างกันเป็นร้อยเท่า
  ทำได้จริงคือ **"ดูเป็นช่วงๆ"** เช่น แคปทุก 5-10 วิ แล้วส่งย้อนหลัง 4-8 เฟรมให้ดูว่าเปลี่ยนอะไรไป
