...
### 🎭 สรรพนาม "ข้าพเจ้า" หลุด — guard ยังจับไม่ได้ (พบ 1 ส.ค. 2569, ยังไม่แก้)
เจอจริงบน Discord: รอสเต้ใช้คำว่า **"ข้าพเจ้า"** แทน "ฉัน" ซึ่งเป็นสรรพนามทางการ/โบราณ
ผิดคาแร็กเตอร์ (รอสเต้เป็นเด็กสาวพูดกันเอง) — ยืนยันแล้วว่า **หลุดทุกด่าน**:
```
persona.fix_persona_slips("ข้าพเจ้าคือรอสเต้")  → "ข้าพเจ้าคือรอสเต้"  (ไม่แก้)
persona.reply_claims_to_be_ai(...)              → False               (ไม่จับ)
```
สาเหตุ: `_MOM_PRONOUN_RE` ครอบแค่ **"ผม"** ตัวเดียว (persona.py:151) ส่วน `fix_persona_slips`
แก้แค่ "ครับ"→"ค่ะ" กับ "ผม"→"ฉัน" — ไม่มีกฎสำหรับสรรพนามทางการรูปอื่น

ที่ควรครอบเพิ่มเมื่อแก้: `ข้าพเจ้า` `กระผม` `ดิฉัน` `หนู`(?) `เรา`(ในบางบริบท)
⚠️ ระวัง false positive: "ข้าพเจ้า" อาจโผล่ในเนื้อหาที่รอสเต้ *อ้างอิง* (ยกคำพูด/อ่านเอกสาร)
ไม่ใช่ที่เธอพูดเอง — ต้องดูบริบทเหมือนที่ `_MOM_PRONOUN_RE` ใช้ blacklist กัน "เส้นผม/ทรงผม"

หมายเหตุ: log ไม่มีหลักฐานเคสนี้ เพราะเนื้อหาข้อความ (PII) อยู่ระดับ DEBUG ไม่เขียนไฟล์
(`grep "ข้าพเจ้า" logs/bot.log` = 0 ผล) — ถ้าจะดีบักเคสแบบนี้ต้องเปิด DEBUG ชั่วคราวก่อน
...
รอบถัดๆ มาเหลือ 1.4-1.6s → น่าจะเป็น cold start ของ ChromaDB/embedding
ผลกระทบ: ผู้ใช้รู้สึกว่ารอสเต้ตอบช้ามากตอนทักครั้งแรกของแต่ละเซสชัน
ทางแก้ที่ยังไม่ได้ลอง: warm-up ตอน startup (query ทิ้ง 1 ครั้ง) แบบเดียวกับที่ทำกับ RVC/F5 worker
...
- **การ์ดจอ 4GB VRAM (RTX 3050 Ti Laptop)** — รันโมเดลใหญ่กว่า 14B ไม่ไหว; qwen3:8b + RVC อยู่พร้อมกันได้ (~3.3GB peak), แต่ STT (Whisper) พร้อมกันอีกจะเต็ม
...
- **VLM (Vision Model) — ตัวที่เล็งไว้ Qwen3-VL-8B ไม่พอดีแน่นอน** — เช็คตัวเลขจริงแล้ว (3 ก.ค. 2569):
  แม้บีบ quantization สุดแล้ว (INT4/Q4_K_M) ตัวโมเดลอย่างเดียวก็ยังกิน ~4.8-6.1GB (ขั้นต่ำที่แนะนำจริงคือ
  8GB, สบายๆ ที่ 12-16GB) **เกิน VRAM ทั้งใบของการ์ดปัจจุบัน (4GB) ก่อนจะนับ RVC ที่ต้องใช้ร่วม (~0.9GB)
  หรือ overhead ตอนประมวลผลรูปจริงด้วยซ้ำ** — ไม่ใช่แค่ "ตึง" แต่ไม่พอดีเลยในทุกกรณี
  - ทางเลือกถ้าต้องการ vision ก่อนอัปเกรดการ์ด: (1) หา VLM ตัวเล็กกว่า เช่น Qwen2-VL-2B (ความฉลาดลดลง
    เยอะ ต้องเช็คว่าพอไหวกับงานที่ต้องการ) (2) ใช้ cloud API เฉพาะ vision (ขัดกับแนวทาง "รันโลคอลทั้งหมด"
    ที่ทำมาตลอด + เสียความเป็นส่วนตัวของรูปที่ส่งไป) (3) รอการ์ดใหม่ (ทางตรงที่สุด)

> ถ้าอัปเกรดการ์ดจอ (VRAM 8-12GB+) หรือใช้เครื่องพิมพ์/อุปกรณ์ WiFi
> หลายข้อจำกัดข้างบนจะเปิดทางได้มากขึ้น (รวมถึง VLM ด้วย — 8GB คือขั้นต่ำที่ Qwen3-VL-8B เริ่มพอไหว)
